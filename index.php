				...
				if( $g_drawdata!='' )
				{
					 ...
					 foreach($g_drawdata as $gdata)
					 {
						...
						{ ?>
							...
							var drawmap<?php echo $counter  ?> = new google.maps.Marker({	
							...
							drawmap<?php echo $counter  ?>.setMap(map);	
							<?php 
						}
						elseif($new_data['type']=='circle')
						{
							  ?>
							var drawmap<?php echo $counter  ?> =new google.maps.Circle({
							strokeWeight: 0,
							type: 'circle',
							fillOpacity: 0.45,
							fillColor: '<?php echo $new_data['color'];  ?>',
							draggable: true,
							map: map,
						...
						elseif($new_data['type']=='polygon')
						{ ?>
							var polygon_cord<?php echo $counter  ?> = [
							<?php  
							...
							var drawmap<?php echo $counter  ?> = new google.maps.Polygon({
							paths: polygon_cord<?php echo $counter  ?>,
							strokeWeight: 0,
							type: 'polygon',
							fillOpacity: 0.45,
							fillColor: '<?php echo $new_data['color'];  ?>',
					...
					<?php }
						elseif( $new_data['type']=='rectangle')
						{
							$bound = json_decode( $new_data['bound'] , true );
							//echo "<pre>";print_r($bound ); die;
						?>
							// Construct the rectangle
							var drawmap<?php echo $counter  ?> = new google.maps.Rectangle({
							strokeWeight: 0,
							type: 'rectangle',
							fillOpacity: 0.45,
							fillColor: '<?php echo $new_data['color'];  ?>',
							draggable: true,
							bounds: {
								north: <?php echo $bound['north'];  ?>,
								south: <?php echo $bound['south'];  ?>,
								east: <?php echo $bound['east'];  ?>,
								west: <?php echo $bound['west'];  ?>
							}
							});
							drawmap<?php echo $counter  ?>.setMap(map);
					<?php }
						  ...
						  { 
							...
							var polyline_cord<?php echo $counter  ?> = [
							<?php  
							foreach($lat_lng as $pol_point )
							{
								echo '{lat: '.$pol_point['lat'].', lng: '.$pol_point['lng'].'},';
							...
							// Construct the polygon.
							var drawmap<?php echo $counter  ?> = new google.maps.Polyline({
								path: polyline_cord<?php echo $counter  ?>,
								type: 'polyline',
								strokeColor: '<?php echo $new_data['color'];  ?>',
								draggable: true,
							});
							drawmap<?php echo $counter  ?>.setMap(map);
				 <?php  } ?>
				 
							google.maps.event.addListener(drawmap<?php echo $counter  ?>, 'click', function() 
							{
								setData( '<?php echo $new_data['type']; ?>' , 'click'  , drawmap<?php echo $counter  ?>  );		
							});
							
							google.maps.event.addListener(drawmap<?php echo $counter  ?>, 'dragstart', function () 
							{
								setData( '<?php echo $new_data['type']; ?>' , 'dragstart'  , drawmap<?php echo $counter  ?>  );		
							});

							google.maps.event.addListener(drawmap<?php echo $counter  ?>, 'dragend', function () 
							{
								setData( '<?php echo $new_data['type']; ?>' , 'dragend'  , drawmap<?php echo $counter  ?>  );		
							});
				 
					 <?php if($new_data['type']=='circle' ){ ?>	
							google.maps.event.addListener(drawmap<?php echo $counter  ?>, 'radius_changed', function() 
							{
									setData( '<?php echo $new_data['type']; ?>' , 'radius_changed'  , drawmap<?php echo $counter  ?>  );	
							});

							google.maps.event.addListener(drawmap<?php echo $counter  ?>, 'center_changed', function() 
							{	
									setData( '<?php echo $new_data['type']; ?>' , 'center_changed'  , drawmap<?php echo $counter  ?>  );	
							});
					<?php }elseif( $new_data['type']=='polygon' || $new_data['type']=='polyline'  ){ ?>
							
							google.maps.event.addListener(drawmap<?php echo $counter  ?>.getPath(), 'insert_at', function() 
							{
									setData( '<?php echo $new_data['type']; ?>' , 'insert_at'  , drawmap<?php echo $counter  ?>  );		
							});

							google.maps.event.addListener(drawmap<?php echo $counter  ?>.getPath(), 'set_at', function() 
							{
								setData( '<?php echo $new_data['type']; ?>' , 'set_at'  , drawmap<?php echo $counter  ?>  );	
							});
					<?php }elseif( $new_data['type']=='rectangle'  ){ ?>
					
							google.maps.event.addListener(drawmap<?php echo $counter  ?>, 'bounds_changed', function() 
							{
								setData( '<?php echo $new_data['type']; ?>' , 'bounds_changed'  , drawmap<?php echo $counter  ?>  );		
							});
					<?php } ?>
				<?php }	
				 }
